...
Module Correctness.
  ...
  *)
  ...
  Inductive directionalProgress `{ep: CharacterInstance Γ Σ}: Direction -> MatchState -> MatchState -> Prop :=
  ...
  Inductive progress `{ep: CharacterInstance Γ Σ}: Direction -> MatchState -> MatchResult -> Prop :=
  ...
  Definition IteratorOn `{ep: CharacterInstance Γ Σ} (str: list Character) (i: Z) := (0 <= i <= Z.of_nat (length str))%Z.
  ...
  Module MatchState. Section main.
    ...
    Definition init `{ep: CharacterInstance Γ Σ} (rer: RegExpRecord) (str: list Character) (i: nat) := (match_state str (Z.of_nat i) (List.repeat None (RegExpRecord.capturingGroupsCount rer))).

    Lemma valid_init `{ep: CharacterInstance Γ Σ}: forall rer str i, (i <= length str) -> Valid str rer (init rer str i).
    Proof.
  ...
  Lemma initialState_validity `{ep: CharacterInstance Γ Σ}: forall input i rer,
    i <= (length input) ->
  ...
  Theorem monotony `{ep: CharacterInstance Γ Σ}: forall r rer input i m,
  ...
  Theorem monotony' `{ep: CharacterInstance Γ Σ}: forall r rer input i m z,
    EarlyErrors.Pass_Regex r nil ->
    countLeftCapturingParensWithin r nil = RegExpRecord.capturingGroupsCount rer ->
    compilePattern r rer = Success m ->
  ...
  Theorem termination `{ep: CharacterInstance Γ Σ}: forall r rer input i m,
    EarlyErrors.Pass_Regex r nil ->
    countLeftCapturingParensWithin r nil = RegExpRecord.capturingGroupsCount rer ->
    compilePattern r rer = Success m ->
  ...
  Theorem no_failure `{ep: CharacterInstance Γ Σ}: forall r rer input i m,
    EarlyErrors.Pass_Regex r nil ->
    countLeftCapturingParensWithin r nil = RegExpRecord.capturingGroupsCount rer ->
    compilePattern r rer = Success m ->
    (i <= length input) ->
