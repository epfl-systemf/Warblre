...
Module Definitions.
  ...
  Definition characterClass_successful_state {Σ} `{ep: CharacterInstance Σ} input endIndex captures (dir: Direction) := input [@ step{dir} endIndex $ captures ].

  Module RepeatMatcher.
    Definition matcher {Σ} `{ep: CharacterInstance Σ} (m: Matcher) (min: non_neg_integer) (max: non_neg_integer_or_inf) 
    ...
    Definition continuation {Σ} `{ep: CharacterInstance Σ} (x: MatchState) (c: MatcherContinuation) (m: Matcher) (min: non_neg_integer) (max: non_neg_integer_or_inf) 
  ...
  Module PositiveLookaround.
    Definition matcher {Σ} `{ep: CharacterInstance Σ} (m: Matcher): MatchState -> MatcherContinuation -> MatchResult :=
    ...
    Lemma lookahead_correctness {Σ} `{ep: CharacterInstance Σ}: forall r ctx rer dir m,
    ...
    Lemma lookbehind_correctness {Σ} `{ep: CharacterInstance Σ}: forall r ctx rer dir m,
  ...
  Module NegativeLookaround.
    ...
    (* Check this definition *)
    Lemma negativeLookahead_correctness {Σ} `{ep: CharacterInstance Σ}: forall r ctx rer dir m,
    ...
    Proof. intros ? ? ? ? ? H. cbn. rewrite -> H. reflexivity. Qed.
    Lemma negativeLookbehind_correctness {Σ} `{ep: CharacterInstance Σ}: forall r ctx rer dir m,
