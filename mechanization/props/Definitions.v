...
Module Definitions.
  ...
  Definition characterClass_successful_state {Character} `{ep: CharacterInstance Character} input endIndex captures (dir: Direction) := input [@ step{dir} endIndex $ captures ].

  Module RepeatMatcher.
    Definition matcher {Character} `{ep: CharacterInstance Character} (m: Matcher) (min: non_neg_integer) (max: non_neg_integer_or_inf) 
    ...
    Definition continuation {Character} `{ep: CharacterInstance Character} (x: MatchState) (c: MatcherContinuation) (m: Matcher) (min: non_neg_integer) (max: non_neg_integer_or_inf) 
  ...
  Module PositiveLookaround.
    Definition matcher {Character} `{ep: CharacterInstance Character} (m: Matcher): MatchState -> MatcherContinuation -> MatchResult :=
    ...
    Lemma lookahead_correctness {Character} `{ep: CharacterInstance Character}: forall r ctx rer dir m,
    ...
    Lemma lookbehind_correctness {Character} `{ep: CharacterInstance Character}: forall r ctx rer dir m,
  ...
  Module NegativeLookaround.
    ...
    (* Check this definition *)
    Lemma negativeLookahead_correctness {Character} `{ep: CharacterInstance Character}: forall r ctx rer dir m,
    ...
    Proof. intros ? ? ? ? ? H. cbn. rewrite -> H. reflexivity. Qed.
    Lemma negativeLookbehind_correctness {Character} `{ep: CharacterInstance Character}: forall r ctx rer dir m,
