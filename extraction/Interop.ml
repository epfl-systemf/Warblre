...
let all_chars: character list = 
  if false then
    ...
    List.init (Unsigned.UInt16.to_int Unsigned.UInt16.max_int) (fun i -> i)
      |> List.map char_of_int (*List.filter_map  (fun i -> if Uchar.is_valid i then Some (char_of_int i) else None)*)
...
type character = Obj.t (* Unsigned.uint16 *)
let char_eq (l: character) (r: character) = Unsigned.UInt16.equal (Obj.magic l) (Obj.magic r)
let char_to_int (c: character): int = Unsigned.UInt16.to_int (Obj.magic c)
let char_of_int (i: int): character = Obj.magic (Unsigned.UInt16.of_int i)
let cmp (l: character) (r: character): int = Unsigned.UInt16.compare (Obj.magic l) (Obj.magic r)
...
(* The two following functions perform conversions from codepoint to character; should they? *)
let code_points_to_string (c: codepoint list): character list = List.flatten (List.map (fun c -> Obj.magic (codepoint_to_utf16 c)) c)
